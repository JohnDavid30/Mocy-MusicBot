 ...
 async function trackStartEventHandler(msgId, channel, player, track, client) {
    try {
        ...
        if(!message) {
            ...
            let savebut = new MessageButton().setCustomId(`save_but_${player.guild}`).setEmoji("💿").setStyle("SECONDARY").setDisabled(false);
            ...
            let embed2 = new MessageEmbed().setColor(message.client.embedColor).setDescription(`[${track.title}](${track.uri}) - \`[${convertTime(track.duration)}]\``).setImage(track.displayThumbnail("3")).setFooter({text: `Requested by ${player.queue.current.requester.username}`, iconURL: player.queue.current.requester.displayAvatarURL({ dynamic: true })});
